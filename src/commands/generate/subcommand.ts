...
function parseSubcommandName(
    ...
    const fileName = `${parent}-${lastPart}`;
    const className = toPascalCase(`${parent}-${lastPart}`);
...
function getDecoratorTemplate(): string {
    ...
    return readFileSync(path.join(templatesDir, 'subcommand.decorator.template'), 'utf-8');
}
...
function handleGenerateSubcommand(options: GenerateOptions): void {
    try {
        if (!options.name) {
            console.error(chalk.red('✗ El nombre es requerido'));
            console.log(chalk.cyan('\nUso: -n <name> o --name <name>'));
            console.log(chalk.gray('\nEjemplo:'));
            console.log(chalk.gray('  patto-cli generate subcommand -n helper -p main'));
            process.exit(1);
        }
        ...
        if (!options.parent) {
            ...
            console.log(chalk.gray('\nEjemplo:'));
            console.log(
                chalk.gray(`  patto-cli generate subcommand -n ${options.name} -p mi-comando`),
            );
            process.exit(1);
        }
        ...
        const { fileName, className, parts } = parseSubcommandName(options.name, options.parent);
        const baseDir = process.cwd();

        if (options.unified) {
            createUnifiedFile(baseDir, fileName, className, options);
            console.log(chalk.green(`\n✓ Subcomando unificado "${fileName}" creado exitosamente!`));
        } else {
            createSplitFiles(baseDir, fileName, className, parts, options);
            console.log(
                chalk.green(
                    `\n✓ Subcomando "${fileName}" creado exitosamente con archivos separados!`,
                ),
            );
        }
    } catch (error) {
        console.error(chalk.red(`\n✗ Error generando el subcomando: ${error}`));
        process.exit(1);
    }
}

export const subcommandSubcommand = new Command('subcommand')
    .alias('s')
    .description('Genera un subcomando')
    .option('-n, --name <name>', 'Nombre del subcomando (en kebab-case)')
    ...
    .option('-d, --description <description>', 'Descripción del subcomando')
    .action((options: GenerateOptions) => handleGenerateSubcommand(options));
